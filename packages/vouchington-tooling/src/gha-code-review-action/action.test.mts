...
describe('code-review action', () => {
  ...
  it('proceeds without the inline comment overlay when absent from the trusted ref, rather than skipping the whole review', () => {
    const build = stepByName.get('Build review prompt')
    expect(build?.run).toContain('has_inline=true')
    expect(build?.run).toContain('has_inline=false')
    expect(build?.run).toContain(
      'Inline comment overlay prompt is not present on the trusted ref; proceeding without it.',
    )
    expect(build?.run).not.toContain(
      'Inline comment overlay prompt is not present on the trusted ref; skipping code review.',
    )
    expect(build?.run).toContain('if [ "$has_inline" = true ]; then')
  })
