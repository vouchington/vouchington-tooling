...
`fetch-repository-paths` requires Node 24+, accepts a regular JSON config up to 256 KiB with at most 64 path mappings, fetches only the schemaVersion-1 config's mapped paths, resolves the requested ref to an immutable SHA, verifies every selected Git blob, and creates private staged output before publishing. Destination and metadata must not already exist. Metadata contains the resolved SHA, sorted file hashes, and deterministic bundle digest.
