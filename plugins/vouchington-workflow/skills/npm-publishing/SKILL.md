...
2. Confirm npm 11.15 or newer, account-level two-factor authentication, write access to the npm
   package or scope, and that the workflow exists on the repository's default branch.
...
4. Build the package as required, then inspect the exact publication payload with
   `npm pack --dry-run <package-directory>`. Stop if it includes secrets, environment files, source
   maps, unrelated workspace files, or missing runtime/type entrypoints.

## Hand off the mutations

Do not run `npm publish` or `npm trust`. Explain that the first command creates an externally visible,
effectively irreversible package version and the second grants the named workflow publish authority.
Tell the human to append a current one-time password after each final `--otp=` without sharing or
recording it. Give these commands with fully resolved values and in this order:
...
Afterward, have the human verify the published package version and run `npm trust list @scope/package`.
If trust already exists, report it rather than revoking or replacing it. Do not retry authorization or
registry failures blindly, and do not fall back to a long-lived npm token without explicit direction.
